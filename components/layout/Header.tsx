      ...
      <div className="max-w-5xl mx-auto px-6 h-12 flex items-center justify-between">
        <Link href="/dashboard" className="flex items-center gap-2.5 group">
          <div className="w-7 h-7 bg-gradient-to-br from-yellow/70 to-yellow/40 rounded-lg flex items-center justify-center transition-transform duration-200 group-hover:scale-105">
            <span className="font-sans font-semibold text-[13px] text-charcoal/60">M</span>
          </div>
          <span className="font-sans font-semibold text-[15px] text-charcoal/80 hidden sm:inline">Menage</span>
        </Link>
        ...
        <nav className="flex items-center gap-0.5">
          {navItems.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href}>
                <span className={`px-3 py-1.5 rounded-lg text-[13px] font-sans font-semibold transition-all duration-200 cursor-pointer ${
                  isActive
          ...
          <button
            ...
            className="text-[12px] font-sans text-charcoal/20 hover:text-charcoal/45 transition-colors duration-200 px-2"
          >
            Quitter
          </button>
        </nav>
