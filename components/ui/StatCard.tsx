...
export function StatCard({ label, value, icon, variant = 'default', subtitle, className }: StatCardProps) {
  if (variant === 'hero') {
    return (
      <div className={cn(
        ...
        "bg-[linear-gradient(135deg,#1a1714,#2d2520,#1a1714)] bg-[length:200%_200%] animate-grad-shift",
        "shadow-lg",
        className
      )}>
        ...
        <div className="flex items-baseline gap-1.5">
          <span className="font-serif text-5xl font-black text-[#F5ECD7] leading-none tracking-tight">{value}</span>
  ...
  return (
    <div className={cn(
      "rounded-[20px] bg-white border border-[#E8E0D4] p-[18px_16px] shadow-sm",
      className
    )}>
      {icon && <div className="text-lg mb-1 opacity-60">{icon}</div>}
