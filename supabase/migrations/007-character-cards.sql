-- ============================================================
-- Migration: Character Card System
-- Extends avatars table and adds weekly character rotation
-- ============================================================

-- 1. Extend avatars table with character card fields
...
CREATE INDEX IF NOT EXISTS idx_weekly_characters_active
    ON weekly_characters(household_id, week_start);
CREATE INDEX IF NOT EXISTS idx_weekly_characters_profile
    ON weekly_characters(profile_id, week_start DESC);
...
CREATE INDEX IF NOT EXISTS idx_character_collection_profile
    ON character_collection(profile_id);

-- 4. RLS Policies
ALTER TABLE weekly_characters ENABLE ROW LEVEL SECURITY;
ALTER TABLE character_collection ENABLE ROW LEVEL SECURITY;

-- weekly_characters: visible to household members
...
-- character_collection: visible to owner
...
-- 5. Seed character data (replace existing basic avatars)
DELETE FROM avatars;
...
(
    'Merlin le Propre',
    'Un sorcier obsede par la proprete magique',
    'wizard',
    'rare',
...
(
    ...
    '{"multiplier": 1.25}',
    'Le pont de son navire brille plus que lor de sa cale. Ses moussaillons recuurrent le pont en chantant.',
    'free',
    true
),
(
    'Chevalier Etincelant',
    'Un chevalier dont armure est un miroir',
    'knight',
    'common',
    '{"primary": "196 163 90", "accent": "212 183 106", "glow": "240 224 160"}',
    'category_bonus',
    '+20% or sur les quetes Sols & Surfaces',
    '{"category": "Sols & Surfaces", "bonus_percent": 20}',
    'Son armure est si polie quon peut y voir son reflet. Il combat la crasse avec la meme ferveur que les dragons.',
    'free',
    true
),
(
    'Fee du Logis',
    'Une fee dont la baguette transforme le desordre en ordre',
    'fairy',
    'epic',
    ...
    'Un coup de baguette et tout se range comme par magie. Les objets semblent danser jusqua leur place.',
    'level',
    true
),
(
    'Alchimiste des Saveurs',
    'Un alchimiste transformant les restes en festins',
    'alchemist',
    'rare',
    '{"primary": "200 139 122", "accent": "212 160 144", "glow": "240 200 184"}',
    'category_bonus',
    '+20% or sur les quetes Cuisine & Vaisselle',
    '{"category": "Cuisine & Vaisselle", "bonus_percent": 20}',
    'Sa cuisine est un laboratoire ou tout ingredient devient or. Meme les restes deviennent un festin royal.',
    'free',
    true
),
(
    'Druide des Jardins',
    'Un druide qui parle aux plantes',
    'druid',
    'common',
    '{"primary": "90 128 96", "accent": "123 155 123", "glow": "168 208 168"}',
    'category_bonus',
    '+20% or sur les quetes Exterieur & Plantes',
    '{"category": "Extérieur & Plantes", "bonus_percent": 20}',
    'Les plantes poussent deux fois plus vite sous ses soins. Il murmure a oreille des fleurs.',
    'free',
    true
),
(
    'Barde Motivant',
    'Un barde dont les chansons inspirent le menage',
    'bard',
    'epic',
    '{"primary": "155 140 181", "accent": "200 184 216", "glow": "224 208 240"}',
    'streak_shield',
    'Protege votre serie 1 fois cette semaine',
    '{"shield_count": 1}',
    'Sa musique donne envie de nettoyer en dansant. Meme les araignees quittent leur toile quand il joue.',
    'level',
    true
),
(
    'Dragon Domestique',
    'Un petit dragon qui brule la poussiere',
    'dragon',
    'legendary',
    '{"primary": "176 112 96", "accent": "212 160 144", "glow": "240 192 168"}',
    'point_multiplier',
    'x1.5 sur tout lor gagne',
    '{"multiplier": 1.5}',
    'Il crache juste assez de feu pour calciner la crasse. Attention a ne pas le laisser pres des rideaux.',
    'level',
    true
),
(
    'Moine de Ordre',
    'Un moine voue au nettoyage methodique',
    'monk',
    'common',
    '{"primary": "139 107 74", "accent": "168 136 96", "glow": "208 184 152"}',
    'time_reduction',
    '-5 min sur les quetes de 20+ min',
    '{"reduction_minutes": 5, "min_duration": 20}',
    'La discipline est sa force, ordre sa devotion. Chaque geste est un acte de priere.',
    'free',
    true
),
(
...
BEGIN
    ...
    -- Add to collection (or increment times_received)
